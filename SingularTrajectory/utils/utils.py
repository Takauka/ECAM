...
def augment_trajectory(obs_traj, pred_traj, flip=True, reverse=True):
    ...
    if flip:
        ...
        flip_tensor = torch.FloatTensor([[[1, -1]]]).to(device)
        
        obs_traj = torch.cat([obs_traj, obs_traj * flip_tensor], dim=0)
        pred_traj = torch.cat([pred_traj, pred_traj * flip_tensor], dim=0)
    elif reverse:
